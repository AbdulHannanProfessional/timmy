...
export default function AdminDashboard() {
  ...
  const verifiedSellers = sellers.filter(s => s.verification_status === 'approved').length;
  ...
  const recentDisputes = disputes.filter(d => d.status !== 'closed').slice(0, 5);
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1">Welcome back! Here's what's happening with your marketplace.</p>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <Link to={createPageUrl('SellerVerification')}>
            <QuickActionCard
              ...
              icon={CheckCircle}
              count={sellers.filter(s => s.verification_status === 'pending').length}
              color="blue"
            />
          </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              {recentOrders.length === 0 ? (
                ...
                recentOrders.map((order) => (
                  <div key={order.id} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                    <div>
                      <p className="font-medium text-slate-900">{order.order_number || `#${order.id.slice(0, 8)}`}</p>
                      <p className="text-sm text-slate-500">{order.card_name}</p>
                    </div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-lg font-semibold">Active Disputes</CardTitle>
            <Link 
          ...
          <CardContent>
            <div className="space-y-4">
              {recentDisputes.length === 0 ? (
                <p className="text-slate-500 text-center py-8">No active disputes</p>
              ) : (
                recentDisputes.map((dispute) => (
                  <div key={dispute.id} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                    <div>
                      <p className="font-medium text-slate-900">{dispute.type?.replace(/_/g, ' ')}</p>
                      <p className="text-sm text-slate-500 truncate max-w-[200px]">{dispute.description}</p>
                    </div>

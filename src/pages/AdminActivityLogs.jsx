...
export default function AdminActivityLogs() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value, row) => (
        <div>
          ...
          {row.entity_id && (
            <p className="text-xs text-slate-500 mt-1 font-mono">{row.entity_id.slice(0, 12)}</p>
          )}
        </div>
      )
    },
    {
      ...
      label: 'Description',
      render: (value) => (
        <span className="text-sm text-slate-600 truncate max-w-[300px] block">{value}</span>
      )
    },
    {
      key: 'ip_address',
      label: 'IP Address',
      render: (value) => <span className="text-sm font-mono text-slate-500">{value || 'N/A'}</span>
    },
    {
      key: 'created_date',
      label: 'Timestamp',

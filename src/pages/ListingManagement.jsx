...
const TCG_CATEGORIES = {
  ...
  mtg: { label: 'Magic: The Gathering', color: 'bg-purple-100 text-purple-700' },
  yugioh: { label: 'Yu-Gi-Oh!', color: 'bg-blue-100 text-blue-700' },
  one_piece: { label: 'One Piece', color: 'bg-red-100 text-red-700' },
  disney_lorcana: { label: 'Disney Lorcana', color: 'bg-indigo-100 text-indigo-700' },
...
export default function ListingManagement() {
  ...
  const totalValue = listings.reduce((sum, l) => sum + (l.price * (l.quantity || 1)), 0);
  ...
  const columns = [
    {
      ...
      render: (value, row) => (
        <div className="flex items-center gap-3">
          {row.image_url ? (
            <img src={row.image_url} alt={value} className="w-12 h-16 object-cover rounded-lg" />
          ) : (
    ...
    },
    {
      ...
      render: (value, row) => (
        <div>
          ...
          {row.market_price && (
            <p className={`text-xs ${
              value < row.market_price * 0.7 ? 'text-red-600' : 
              value > row.market_price * 1.3 ? 'text-amber-600' : 'text-slate-500'
            }`}>
  ...
  const actions = [
    { label: 'View Details', icon: Eye, onClick: (row) => { setSelectedListing(row); setShowModal(true); }},
    { label: 'Approve', icon: CheckCircle, onClick: handleApprove },
    { label: 'Reject', icon: XCircle, onClick: (row) => { setSelectedListing(row); setShowModal(true); }, destructive: true },
  ...
  const filters = [
    {
      key: 'tcg_category',
      label: 'Category',
      options: Object.entries(TCG_CATEGORIES).map(([value, { label }]) => ({ value, label }))
    },
    {
      key: 'status',
      label: 'Status',
          ...
          <div className="space-y-6">
            <div className="flex gap-6">
              ...
              <div className="flex-1 space-y-4">
                <div>
                  <h3 className="text-xl font-bold text-slate-900">{selectedListing.card_name}</h3>
                  <p className="text-slate-500">{selectedListing.card_set} • {selectedListing.card_number}</p>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-sm text-slate-500">Price</p>
                    <p className="text-2xl font-bold text-slate-900">${selectedListing.price?.toFixed(2)}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium capitalize">{selectedListing.condition?.replace(/_/g, ' ')}</p>
                  </div>
                  <div>
                    <p className="text-sm text-slate-500">Quantity</p>
                    <p className="font-medium">{selectedListing.quantity || 1}</p>
                  </div>
                  <div>
                    <p className="text-sm text-slate-500">Category</p>
                    <Badge className={TCG_CATEGORIES[selectedListing.tcg_category]?.color}>
                  ...
                  <Card className="bg-red-50 border-red-200">
                    <CardContent className="py-3">
                      <div className="flex items-center gap-2 text-red-700">
                        ...
                        <span>{selectedListing.flag_reason || 'Suspicious activity detected'}</span>
                      </div>
                    </CardContent>
                  </Card>
                )}

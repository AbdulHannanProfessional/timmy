...
export default function DisputeManagement() {
  ...
  const resolvedDisputes = disputes.filter(d => d.status.includes('resolved') || d.status === 'closed');
  const totalRefunded = disputes.reduce((sum, d) => sum + (d.refund_amount || 0), 0);
  ...
  const columns = [
    ...
    },
    {
      key: 'order_id',
      label: 'Order',
      render: (value) => <span className="text-sm font-mono">{value?.slice(0, 8)}</span>
    },
    {
      key: 'status',
      label: 'Status',
    ...
    if (selectedDispute) {
      updateMutation.mutate({
        ...
        data: { 
          ...
          refund_amount: decision === 'resolved_buyer' ? parseFloat(refundAmount) || 0 : 0
        }
      });
    }
  };
  ...
  const actions = [
    { label: 'View Details', icon: Eye, onClick: (row) => { setSelectedDispute(row); setShowModal(true); }},
    { label: 'Start Review', icon: Scale, onClick: (row) => updateMutation.mutate({ id: row.id, data: { status: 'under_review' }}) }
  ];
  ...
  return (
    <div>
      ...
      <DetailModal
        open={showModal}
        onClose={() => { setShowModal(false); setResolution(''); setRefundAmount(''); }}
        ...
        {selectedDispute && (
          <div className="space-y-6">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-slate-500 mt-2">
                  Opened {selectedDispute.created_date && format(new Date(selectedDispute.created_date), 'MMMM d, yyyy')}
                </p>
              </div>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-slate-700">{selectedDispute.description || 'No description provided'}</p>
              </CardContent>
            </Card>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-2">
                {selectedDispute.evidence?.length > 0 ? (
                  ...
                  <p className="text-slate-500 text-center py-4">No evidence submitted</p>
                )}
              </div>
            </div>
            ...
            {selectedDispute.resolution && (
              <Card className="bg-emerald-50 border-emerald-200">
                <CardContent className="py-3">
                  ...
                  <p className="text-emerald-600">{selectedDispute.resolution}</p>
                  {selectedDispute.refund_amount > 0 && (
                    <p className="text-emerald-700 mt-2">Refunded: ${selectedDispute.refund_amount.toFixed(2)}</p>
                  )}
                </CardContent>
              </Card>
            )}

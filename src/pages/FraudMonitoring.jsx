...
const ALERT_TYPES = {
  suspicious_listing: { label: 'Suspicious Listing', icon: Flag, color: 'text-orange-600' },
  price_anomaly: { label: 'Price Anomaly', icon: TrendingUp, color: 'text-amber-600' },
  multiple_accounts: { label: 'Multiple Accounts', icon: Users, color: 'text-purple-600' },
  scan_anomaly: { label: 'Scan Anomaly', icon: Zap, color: 'text-blue-600' },
  volume_spike: { label: 'Volume Spike', icon: TrendingUp, color: 'text-cyan-600' },
  chargeback: { label: 'Chargeback', icon: XCircle, color: 'text-red-600' },
  ip_mismatch: { label: 'IP Mismatch', icon: AlertTriangle, color: 'text-pink-600' }
};
...
export default function FraudMonitoring() {
  ...
  const columns = [
      ...
      render: (value) => {
        ...
        return (
          <div className="flex items-center gap-2">
            <Icon className={`w-4 h-4 ${alertInfo?.color || 'text-slate-600'}`} />
    ...
    },
    {
      ...
      label: 'Description',
      render: (value) => (
        <span className="text-sm text-slate-600 truncate max-w-[200px] block">{value}</span>
      )
    },
    {
      key: 'status',
  ...
  const actions = [
    { label: 'Investigate', icon: Eye, onClick: (row) => { setSelectedAlert(row); setShowModal(true); }},
    { label: 'Mark Resolved', icon: CheckCircle, onClick: (row) => updateMutation.mutate({ id: row.id, data: { status: 'resolved' }}) },
    { label: 'Dismiss', icon: XCircle, onClick: (row) => updateMutation.mutate({ id: row.id, data: { status: 'dismissed' }}) }
  ];
  ...
  const filters = [
    ...
    },
    {
      key: 'alert_type',
      label: 'Type',
      options: Object.entries(ALERT_TYPES).map(([value, { label }]) => ({ value, label }))
    }
  ];
  ...
  return (
    <div>
      ...
      <DetailModal
        open={showModal}
        onClose={() => { setShowModal(false); setResolutionNotes(''); }}
          ...
          <div className="space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-500">
                    Detected {selectedAlert.created_date && format(new Date(selectedAlert.created_date), 'MMMM d, yyyy h:mm a')}
                  </p>
                </div>
              </div>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-sm font-medium text-slate-500">Alert Details</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-slate-500">User</span>
                  <span className="font-medium">{selectedAlert.user_email}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">IP Address</span>
                  <span className="font-mono">{selectedAlert.ip_address || 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                ...
                {selectedAlert.related_entity_id && (
                  <div className="flex justify-between">
                    <span className="text-slate-500">Related {selectedAlert.related_entity_type}</span>
                    <span className="font-mono">{selectedAlert.related_entity_id.slice(0, 12)}</span>
                  </div>
                )}
              </CardContent>
            </Card>
                ...
                <>
                  {selectedAlert.status === 'new' && (
                    <Button 
                      ...
                      onClick={() => updateMutation.mutate({ id: selectedAlert.id, data: { status: 'investigating' }})}
                    >

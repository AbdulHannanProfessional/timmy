...
export default function PayoutManagement() {
  ...
  const totalPending = pendingPayouts.reduce((sum, p) => sum + (p.net_amount || 0), 0);
  const totalCompleted = completedPayouts.reduce((sum, p) => sum + (p.net_amount || 0), 0);
  ...
  const columns = [
    ...
    },
    {
      key: 'amount',
      label: 'Gross',
      render: (value) => <span className="text-slate-600">${value?.toFixed(2)}</span>
    },
    {
      key: 'platform_fee',
      label: 'Fee',
      render: (value) => <span className="text-slate-500">${value?.toFixed(2)}</span>
    },
    {
      key: 'net_amount',
      label: 'Net Payout',
      render: (value) => <span className="font-semibold text-emerald-600">${value?.toFixed(2)}</span>
    },
    {
      key: 'payout_method',
      label: 'Method',
  ...
  const actions = [
    { label: 'View Details', icon: Eye, onClick: (row) => { setSelectedPayout(row); setShowModal(true); }},
    ...
    <div>
      ...
      <Tabs defaultValue="pending" className="space-y-6">
        <TabsList className="bg-slate-100">
          ...
          <TabsTrigger value="delayed">Delayed ({delayedPayouts.length})</TabsTrigger>
      ...
      <DetailModal
        open={showModal}
        onClose={() => { setShowModal(false); setDelayReason(''); }}
        ...
        {selectedPayout && (
          <div className="space-y-6">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-lg font-bold text-slate-900">{selectedPayout.seller_email}</h3>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-medium text-slate-500">Payout Breakdown</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-red-600">-${selectedPayout.platform_fee?.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between font-bold pt-2 border-t">
                    <span>Net Payout</span>
                    <span className="text-emerald-600">${selectedPayout.net_amount?.toFixed(2)}</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="flex justify-between">
                    ...
                    <span className="capitalize">{selectedPayout.payout_method?.replace(/_/g, ' ')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Scheduled</span>
                    <span>{selectedPayout.scheduled_date ? format(new Date(selectedPayout.scheduled_date), 'MMM d, yyyy') : 'N/A'}</span>
                  </div>
                  {selectedPayout.completed_date && (
                    <div className="flex justify-between">
                      <span className="text-slate-500">Completed</span>
                      <span>{format(new Date(selectedPayout.completed_date), 'MMM d, yyyy')}</span>
                    </div>
                  )}
                </CardContent>
              </Card>
            ...
            {selectedPayout.delay_reason && (
              <Card className="bg-amber-50 border-amber-200">
                <CardContent className="py-3">
                  ...
                  <p className="text-amber-600">{selectedPayout.delay_reason}</p>
                </CardContent>
              </Card>
            )}

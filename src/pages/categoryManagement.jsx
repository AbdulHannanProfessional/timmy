      ...
      {isLoading ? (
        ...
        <Card className="text-center py-12">
          <CardContent>
            ...
            <p className="text-slate-500 mb-4">Add your first TCG category to get started</p>
            ...
            <Card key={category.id} className="overflow-hidden">
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between">
                  ...
                  <Badge className={category.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}>
                      ...
                      <div className="mt-2 pl-6 space-y-1">
                        {category.sets.slice(0, 5).map((set, idx) => (
                          <p key={idx} className="text-sm text-slate-600">
                            {set.name} <span className="text-slate-400">({set.code})</span>
                          </p>
                        ))}
                        {category.sets.length > 5 && (
                          <p className="text-sm text-slate-400">+{category.sets.length - 5} more</p>
                        )}
                      </div>
                    )}
                  </div>
